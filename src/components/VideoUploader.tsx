...
export const VideoUploader: React.FC<VideoUploaderProps> = ({
  ...
  useEffect(() => {
    ...
    fetch('/api/evaluator-status', { signal: controller.signal }).then(r => r.ok ? r.json() : null).then(s => { setAiAvailable(s?.available === true); setAiProvider(s?.provider || ''); }).catch(() => { if (!controller.signal.aborted) setAiAvailable(false); });
  ...
  // Preserve the upload/preview flow; only the evaluator transport changes.
  const handleEvaluate = async () => {
    if (!videoUrl || isEvaluating) return;
    ...
    try {
      ...
      const response = await fetch('/api/evaluate-reel', {
        ...
        body: JSON.stringify({ title: videoTitle, niche, captionInput, videoConcept, audioType, fileFormat, fileSizeMb, language, evidence }),
      });
      const contentType = response.headers.get('content-type') || '';
      if (!contentType.includes('application/json')) throw new Error(language === 'ko' ? '이 사이트의 영상 분석 서버가 연결되어 있지 않습니다. 임의의 점수를 만들지 않습니다.' : 'The video analysis server is not connected to this site. No substitute rating was generated.');
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm text-slate-800 min-w-0">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-6 border-b border-gray-100 pb-4">
        ...
        <span className="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700">
          {language === 'ko' ? '영상 관찰 확인 · v2' : 'Video-grounded review · v2'}
        </span>
      </div>

      {aiAvailable === false && <p role="status" className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">{language === 'ko' ? '영상 업로드와 미리보기를 사용할 수 있습니다. 실제 AI 분석은 로컬 서버의 API 연결을 설정한 후 사용할 수 있습니다. 임의의 점수는 표시하지 않습니다.' : 'Upload and preview are available. Real AI analysis needs the local server’s API connection configured. No simulated ratings will be shown.'}</p>}
      {aiAvailable && aiProvider === 'local-codex' && <p role="status" className="mb-5 rounded-xl border border-indigo-100 bg-indigo-50 p-3 text-sm text-indigo-800">{language === 'ko' ? '로컬 Codex 분석 연결됨 · 업로드한 영상의 장면을 검토합니다.' : 'Local Codex analysis connected · reviews footage from your upload.'}</p>}
      {groundingFailure && <div className="mb-5"><WhatISaw report={groundingFailure} language={language} /></div>}
        ...
        <div className="device-uploader-details min-w-0 flex flex-col justify-between gap-4">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={niche}
                onChange={(e) => { if (isTargetNiche(e.target.value)) setNiche(e.target.value); setGroundingFailure(null); }}
          ...
          {(videoError || evaluationError) && (
            <div role="alert" className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-semibold leading-relaxed text-red-700">
              ...
              <span>{videoError || evaluationError}</span>
            </div>
          )}
