...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  const rubricHistory = history.filter(item => (item.evaluatorVersion || 'legacy') === (history[0]?.evaluatorVersion || 'legacy'));
  const chronologicalHistory = [...rubricHistory].reverse();
  ...
  const previousReel = rubricHistory[1];
  const averageScore = rubricHistory.length
    ? rubricHistory.reduce((acc, curr) => acc + curr.overallStars, 0) / rubricHistory.length
    : 0;
  const bestScore = rubricHistory.length ? Math.max(...rubricHistory.map((item) => item.overallStars)) : 0;
  ...
  const mixedRubrics = new Set(history.map(item => item.evaluatorVersion || 'legacy')).size > 1;
  return (
    <div className="space-y-6 animate-fadeIn">
      {mixedRubrics && <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{language === 'ko' ? '이 기록에는 이전 평가 기준이 포함되어 있습니다. 서로 다른 기준의 점수 차이를 개선 효과로 해석하지 마세요.' : 'This history includes older scoring rules. Differences across rubric versions do not measure editing improvement.'}</p>}
      {/* Header Banner */}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-400"><Zap className="h-4 w-4 text-violet-500" /> {language === 'ko' ? '최신 팔로우 잠재력' : 'Latest follow potential'}</div>
          <div className="mt-2 text-2xl font-extrabold text-slate-900">{latestReel?.followerGrowthPotentialPercent != null ? `${latestReel.followerGrowthPotentialPercent}${latestReel.evaluatorVersion ? '/100' : '%'}` : '—'}</div>
        </div>
      </div>
        ...
        <div className="device-dashboard-primary min-w-0 bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            ...
            <span className="text-[11px] text-slate-400 font-medium">
              {language === 'ko' ? '동일한 평가 기준의 기록' : 'History using the same scoring rules'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="bg-slate-50 border border-gray-200 rounded-2xl p-4 space-y-2">
              ...
              <div className="text-xs space-y-1 text-slate-700">
                <div>
                  {t('expectedSkipRate')}: <span className="font-bold text-indigo-600">{reel1.prediction ? `${reel1.prediction.low}–${reel1.prediction.high}%` : `${reel1.expectedSkipRatePercent}%`}</span>
                </div>
                <div>
                  {language === 'ko' ? '팔로우 잠재력:' : 'Follow potential:'} <span className="font-bold text-slate-800">{reel1.followerGrowthPotentialPercent == null ? '—' : `${reel1.followerGrowthPotentialPercent}${reel1.evaluatorVersion ? '/100' : '%'}`}</span>
                </div>
              </div>
            </div>
              ...
              <div className="text-xs space-y-1 text-slate-700">
                <div>
                  ...
                  <span className="font-bold text-green-600">
                    {reel2.prediction ? `${reel2.prediction.low}–${reel2.prediction.high}%` : `${reel2.expectedSkipRatePercent}%`}
                  </span>{' '}
                  ({(reel2.evaluatorVersion || 'legacy') !== (reel1.evaluatorVersion || 'legacy')
                    ? (language === 'ko' ? '평가 기준이 다름' : 'different scoring rules')
                    : reel2.prediction ? (language === 'ko' ? '참고용 추정 범위' : 'heuristic estimate range')
                ...
                <div>
                  {language === 'ko' ? '팔로우 잠재력:' : 'Follow potential:'} <span className="font-bold text-slate-800">{reel2.followerGrowthPotentialPercent == null ? '—' : `${reel2.followerGrowthPotentialPercent}${reel2.evaluatorVersion ? '/100' : '%'}`}</span>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <tr key={reel.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-3 font-bold text-slate-800">
                      <span className={reel.expectedSkipRatePercent <= 20 ? 'text-green-600' : 'text-amber-600'}>
                        {reel.prediction ? `${reel.prediction.low}–${reel.prediction.high}%` : `${reel.expectedSkipRatePercent}%`}
                      </span>
                    </td>
                    <td className="p-3 font-bold text-indigo-600">{reel.followerGrowthPotentialPercent == null ? '—' : `${reel.followerGrowthPotentialPercent}${reel.evaluatorVersion ? '/100' : '%'}`}</td>
                    <td className="p-3">
