...
export function createLocalEvaluation(input: AuditInput): ReelEvaluation {
  ...
  edits.push({
    id: 'written-hook', timestampRange: `0–${sec(Math.min(.8, duration))}`, type: 'hook', severity: 'recommended',
    issue: ko
  ...
  if (activeVideo && (m?.sceneCutScore ?? 0) >= 35) edits.push({
    id: 'busy-cuts', timestampRange: `${sec(Math.max(0, strongestTime - .5))}–${sec(Math.min(duration, strongestTime + .5))}`, type: 'cut', severity: 'recommended',
  ...
  if (!activeVideo || staticDuration >= .8) edits.push({
    id: 'transition-style', timestampRange: staticRange, type: 'cut', severity: staticDuration >= 1.5 ? 'critical' : 'recommended',
  ...
  if ((m?.payoffChangeScore ?? 0) < 55) edits.push({ id: 'payoff', timestampRange: `${sec(duration * .72)}–${sec(duration)}`, type: 'payoff', severity: (m?.payoffChangeScore ?? 0) < 18 ? 'critical' : 'recommended', issue: ko ? `마지막 화면이 앞 장면과 충분히 구분되지 않아 결과가 약하게 보입니다.` : `The ending does not separate clearly enough from the preceding visuals, weakening the result.` , solution: ko ? `가장 변화가 큰 장면에서 마지막 결과로 하드컷하고 화면에는 “최종 결과” 또는 “전 / 후”만 표시하세요.` : `Hard-cut from the strongest-change shot to the final result. Use only “Final result” or “Before / After” on screen.` });
  ...
  edits.push({
    id: 'payoff-card', timestampRange: `${sec(duration * .85)}–${sec(duration)}`, type: 'payoff', severity: 'recommended',
  ...
  const guidance: NonNullable<ReelEvaluation['stanceByStanceGuidance']> = [
    { durationRange: `0–${p1}s`, stanceTheme: ko ? '문제 제시 & 훅' : 'Problem & hook', optionAHookText: captions.recommendedHooks[0], optionBHookText: captions.recommendedHooks[1], optionCHookText: captions.recommendedHooks[2], onScreenGuidance: ko ? `이 업로드에서 시작부 변화가 가장 큰 ${sec(openingTime)} 장면을 먼저 보여주고, 피사체 옆 빈 공간에 굵은 흰색 훅을 배치하세요.` : `Lead with this upload’s strongest opening change near ${sec(openingTime)} and place bold white hook text in the open space beside the subject.` },
    { durationRange: `${p1}–${p2}s`, stanceTheme: ko ? '과정 & 시각적 증명' : 'Process & visual proof', optionAHookText: ko ? `${shortSubject}의 핵심 변화입니다.` : `Here is the key change in ${shortSubject}.`, optionBHookText: ko ? `어떤 디테일이 먼저 보이나요?` : `Which detail do you notice first?`, optionCHookText: ko ? `이 장면 하나가 결과를 바꿉니다.` : `This one shot changes the result.`, onScreenGuidance: ko ? `가장 큰 화면 변화가 나타나는 ${sec(strongestTime)} 전후에는 하드컷을 사용하세요. ${staticDuration >= .8 ? `${staticRange}의 정적인 부분은 디테일 컷이나 펀치인으로 바꾸세요.` : '현재의 빠른 장면 연결은 유지하세요.'}` : `Use a hard cut around the largest visual change near ${sec(strongestTime)}. ${staticDuration >= .8 ? `Replace the static stretch at ${staticRange} with a detail shot or punch-in.` : 'Keep the current quick shot connection.'}` },
    { durationRange: `${p2}–${r1(duration)}s`, stanceTheme: ko ? '결과 & 행동 유도' : 'Payoff & call to action', optionAHookText: captions.valueCTA, optionBHookText: captions.cliffhangerCTA, optionCHookText: captions.commentBaitQuestion, onScreenGuidance: ko ? `마지막 결과 화면에 CTA를 하단 중앙에 배치하고, 첫 장면과 같은 크롭·밝기로 맞춰 자연스럽게 반복되게 하세요.` : `Place the CTA at the lower center of the final result shot, then match its crop and brightness to the opening so replay feels continuous.` },
  ];
