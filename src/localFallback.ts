...
export function createLocalEvaluation(input: AuditInput): ReelEvaluation {
  ...
  const captions: ReelEvaluation['captionOptimization'] = ko ? {
    recommendedHooks: [activeVideo ? '놓치면 다시 봐야 합니다.' : '작은 변화 하나가 전체 화면을 바꿉니다.', highContrast ? '빛과 그림자가 바뀌는 순간을 보세요.' : '어느 디테일이 먼저 달라졌을까요?', (m?.payoffChangeScore ?? 0) < 55 ? '마지막 결과를 먼저 확인하세요.' : '끝까지 보면 첫 장면이 다르게 보입니다.'],
    valueCTA: `가장 눈에 띈 화면 변화가 무엇인지 확인하고, 편집 참고용으로 저장하세요.`, cliffhangerCTA: `다음 편집본에서는 ${staticDuration >= .8 ? '정체 구간을 줄인 버전' : '더 자연스러운 루프 버전'}을 비교해 보여드리겠습니다.`, commentBaitQuestion: `가장 먼저 눈에 들어온 변화는 구도, 움직임, 밝기 중 무엇인가요?`, targetHashtags: [formatTag, paceTag, lightTag, '#릴스편집', '#영상분석']
  } : {
    recommendedHooks: [activeVideo ? `Don't blink—you may need to watch this twice.` : `One small visual change reshapes the whole frame.`, highContrast ? `Watch the light and shadow shift.` : `Which detail changes first?`, (m?.payoffChangeScore ?? 0) < 55 ? `See the final result first.` : `The ending changes how the opening looks.`],
    valueCTA: `Notice which visual change catches your eye first, then save this as an editing reference.`, cliffhangerCTA: `Next, compare ${staticDuration >= .8 ? 'a version with the visual hold tightened' : 'a version with a smoother loop'}.`, commentBaitQuestion: `What caught your eye first: composition, movement, or lighting?`, targetHashtags: [formatTag, paceTag, lightTag, '#ReelsEditing', '#VideoAnalysis']
  };
