...
import type { EvaluationRequest, Observations, Feedback, ScoreResult, GroundedContext, GrowthPrediction } from './contracts';
...
export function adaptEvaluation(request: EvaluationRequest, o: Observations, s: ScoreResult, f: Feedback, id: string, transcriptAvailable: boolean, context: GroundedContext): ReelEvaluation {
  ...
  const modelledGrowthMidpoint = s.conversionIndex ?? Math.round(s.overallScorePercent / 5) * 5;
  const growthConfidence = s.conversionIndex !== null && o.confidence === 'high' ? 'high' : 'medium';
  const growthWidth = growthConfidence === 'high' ? 15 : 25;
  const growthPrediction: GrowthPrediction = {
    low: Math.max(0, modelledGrowthMidpoint - growthWidth), high: Math.min(100, modelledGrowthMidpoint + growthWidth), midpoint: modelledGrowthMidpoint,
    confidence: growthConfidence, basis: 'heuristic_not_empirically_calibrated' as const,
    explanation: s.conversionIndex === null ? 'Broad range derived from the five grounded creative aspect ratings because conversion-specific semantic evidence was unavailable.' : 'Range centred on the deterministic conversion-potential index derived from grounded hook and payoff evidence.',
  };
  ...
  return {
    ...
    evaluatorVersion: s.version, prediction: s.skipEstimate, growthPrediction, conversionIndex: s.conversionIndex,
    ...
    expectedSkipRatePercent: s.skipEstimate.midpoint, followerGrowthPotentialPercent: growthPrediction.midpoint, nonFollowerInterestStars: s.nonFollowerInterestStars, shareabilitySendScore: s.shareabilityIndex,
    aspects: {
      ...
      technicalCompliance: { stars: a.technicalCompliance, label: 'Technical & Unconnected Reach', watermarkDetected: o.watermark === 'unknown' ? null : o.watermark === 'present', resolutionText: `${request.evidence.width} × ${request.evidence.height}`, safeZoneViolation: o.safeZone === 'unknown' ? null : o.safeZone === 'violation', captionQuality: f.textObservation || notVerified, verdict: f.aspectNotes.technicalCompliance.verdict },
    },
